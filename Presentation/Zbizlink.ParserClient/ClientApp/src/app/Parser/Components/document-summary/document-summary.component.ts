...
export class DocumentSummaryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //this.GetProposalSourcesSynonymsList();
  };
  ...
  drawFields(summaryFieldArray: Array<SummaryField>) {
    this.removeControlIfExist(this.groupForm);
    debugger;

    // let mandatoryfields: Array<SummaryField>;
    // if (summaryFieldArray !== undefined && summaryFieldArray !== null && summaryFieldArray.length > 0) {
    //   mandatoryfields = this.getMandatoryFields(summaryFieldArray)
    // }
  ...
  GetProposalSourcesSynonymsList() {
    this.rfpManipulationWebApiService.getProposalSourcesSynonymsList().subscribe(
      (response) => {
        if (response.code === 1) {
          this.agencyList = response.response.AgencyList;
          this.contractVehicleList = response.response.ContractVehicleList;
          this.industryList = response.response.IndustryList;
          this.stateList = response.response.StatesList;
        }
      },
  ...
  getMandatoryFields(summaryFieldArray: Array<SummaryField>): SummaryField[] {
    let summaryFields: Array<SummaryField> = new Array<SummaryField>();

    summaryFieldArray.forEach(
      control => {

        if (control.Mandatory === true && control.FieldDisplayName !== "Solicitation Title" && control.FieldDisplayName !== "Solicitation Number" &&
          control.FieldDisplayName !== "Requesting Agency" && control.FieldDisplayName !== "Original Posted Date" &&
          control.FieldDisplayName !== "Closing Date and Time") {
          summaryFields.push(control);
        }
      });
