using System;
using System.Collections.Generic;
using System.Text;

namespace Zdaas.RFPCommon.ListNumbers.Contracts
{
    public class RomanUpperDot : IListNumberCreation
    {
        public bool FirstLetter(string number)
        {
            if(number == "I.")
            {
                return true;
            }

            return false;
        }

        public string GetChild(string number)
        {
            throw new NotImplementedException();
        }

        public string GetSibling(string number)
        {
            number = number.Substring(0, number.LastIndexOf('.'));

            int index = Array.IndexOf(Utility.romanUpperArray, number);

            if (index != -1 && (index < (Utility.romanUpperArray.Length - 1)))
            {
                return Utility.romanUpperArray[index + 1] + ".";
            }

            return "";
        }
    }
}
